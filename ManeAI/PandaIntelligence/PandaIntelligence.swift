...
final class PanelManager: NSObject, ObservableObject, NSWindowDelegate {
    ...
    /// The onboarding window - shown on first launch.
    /// This intentionally uses normal macOS window chrome, unlike the command palette.
    private(set) var onboardingPanel: NSWindow?

    /// The primary application window. This is intentionally managed by
    /// AppKit so Dock activation and reopening always restore a real window.
    ...
    /// Keeps user-selected library folders current after the initial scan.
    var fileMonitoringService: FileMonitoringService?
    ...
    private func setupPanel() {
        ...
        let monitoringService = FileMonitoringService(indexingService: indexService)
        self.fileMonitoringService = monitoringService
        monitoringService.restorePersistedWatches()
    ...
    private func setupOnboardingPanel() {
        // Onboarding is a regular app window, not a Raycast-style overlay. This gives
        // people the familiar red/yellow/green window controls and normal app behavior.
        let panel = NSWindow(
        ...
        panel.title = "Panda Intelligence Setup"
    ...
    private func setupMainWindow() {
        guard let container = modelContainer,
              ...
              let monitoringService = fileMonitoringService else { return }
        ...
        window.title = "Panda Intelligence"
        ...
        let contentView = ContentView()
            ...
            .modelContainer(container)
            // The legacy library screens contain nested split views. Their
            // automatic toolbar items conflict in a manually hosted window;
            // the new product toolbar will be added with the redesigned UI.
    ...
    func completeOnboarding() {
        ...
        // The Dock icon should always lead to the full native app. The
        // floating panel remains available only through the global shortcut.
    ...
    func windowWillClose(_ notification: Notification) {
        guard let window = notification.object as? NSWindow, window === onboardingPanel else { return }
        isOnboardingVisible = false
    }
    ...
    private func setupHotkey() {
        // Onboarding owns the permission prompt. Calling the prompting API on
        // launch can show an alert before users see why the permission is needed.
        let accessEnabled = AXIsProcessTrusted()

        if !accessEnabled {
            print("⚠️ Panda Intelligence needs Accessibility permission for the global hotkey.")
        }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        print("✅ Panda Intelligence launched - Press Ctrl+W for quick search")
    }

    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        PanelManager.shared.showMainWindow()
        return true
    }
...
struct ManeAIApp: App {
    ...
    var body: some Scene {
        // The menu-bar item and Ctrl+W both open the compact Spotlight-style
        // search panel. The main native window is managed by PanelManager.
        MenuBarExtra {
...
struct MenuBarView: View {
    ...
    var body: some View {
        Group {
            ...
            Button("Open Panda Intelligence") {
                panelManager.showMainWindow()
            }
            ...
            Button("Quit Panda Intelligence") {
                NSApp.terminate(nil)
            }
...
struct RaycastPanelContent: View {
    ...
    private let quickActions: [(title: String, subtitle: String, icon: String, color: Color, id: String, useAssetImage: Bool)] = [
        ...
        ("AI Chat", "Panda Intelligence AI", "icon", Color(red: 0.95, green: 0.3, blue: 0.35), "chat", true),
    ...
    private var placeholderText: String {
        ...
        case .chat: return "Ask Panda Intelligence anything..."
    ...
    private func iconForFile(_ path: String, mediaType: MediaType?) -> (String, Color) {
        ...
        if let type = mediaType {
            ...
            case .video:
                return ("film.fill", Color(red: 0.55, green: 0.45, blue: 0.95))
            case .text:
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: message.isUser ? .trailing : .leading, spacing: 4) {
                Text(message.isUser ? "You" : "Panda Intelligence")
