...
struct OnboardingView: View {
    ...
    // File access is granted when someone selects a folder or file. Requiring
    // Full Disk Access here is unnecessary and macOS cannot grant it on the
    // app's behalf, so onboarding stays focused on permissions we can request.
    ...
    var body: some View {
        ...
        // The accessibility switch is changed in another app. Re-check it as
        // soon as Panda Intelligence becomes active again instead of asking for a restart.
    ...
    private func refreshAccessibilityStatus() {
        ...
        guard accessibilityGranted != isTrusted else { return }
            ...
            VStack(spacing: 28) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text(accessibilityGranted
                         ? "Panda Intelligence is ready. Press Ctrl+W to start."
                         : "Needed only for the Ctrl+W global shortcut.")
                        .font(.system(size: 15))
                        .foregroundStyle(Color(white: 0.5))
                        .multilineTextAlignment(.center)
                }
                ...
                Group {
                    if !accessibilityGranted {
                        VStack(spacing: 12) {
                            ...
                            Text("Choose Open System Settings, turn on Panda Intelligence, then return here.")
                                .font(.system(size: 12))
