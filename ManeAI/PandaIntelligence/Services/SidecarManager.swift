...
class SidecarManager: ObservableObject {
    ...
    private var dbPath: String {
        ...
        let maneAIDir = appSupport.appendingPathComponent("PandaIntelligence", isDirectory: true)
    ...
    init(port: Int? = nil) {
        // Distribution builds include the sidecar and reserve port 3002 for it.
        // Xcode builds intentionally use the local development backend, which
        // is started with `pnpm start:dev` on port 3000.
        let bundledSidecarExists = Bundle.main.path(
            forResource: "main",
            ofType: "js",
            inDirectory: "sidecar/dist"
        ) != nil
        let resolvedPort = port ?? (bundledSidecarExists ? 3002 : 3000)
        self.port = resolvedPort
        self.baseURL = URL(string: "http://127.0.0.1:\(resolvedPort)")!
    }
