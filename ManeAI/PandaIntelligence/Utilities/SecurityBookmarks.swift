...
/// Keeps selected library folders available on both sandboxed development
/// builds and direct-distribution builds with Full Disk Access.
class SecurityBookmarks {

    static let shared = SecurityBookmarks()

    private let bookmarksKey = "PandaIntelligence.SecurityBookmarks"
    ...
    func listFiles(in directoryURL: URL, extensions: [String]? = nil) throws -> [URL] {
        // Direct-distribution builds are not App-Sandboxed. Reading a folder
        // selected through NSOpenPanel should work without a scoped-resource
        // token; requiring one here would incorrectly reject valid folders.
        if let direct = try? listFilesInternal(in: directoryURL, extensions: extensions) {
            return direct
        }
    ...
    func listFilesRecursively(in directoryURL: URL, extensions: [String]? = nil) throws -> [URL] {
        if let direct = try? listFilesRecursivelyInternal(in: directoryURL, extensions: extensions) {
            return direct
        }
        guard startAccessing(directoryURL) else {
            if let resolvedURL = resolveBookmark(for: directoryURL.path) {
